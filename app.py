...
def get_timeline(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Получает ленту твитов.
    Сначала твиты с лайками от подписок (по убыванию лайков),
    затем все остальные твиты (новые сверху).
    """
    following_ids = {f.following_id for f in user.following}
    all_tweets = db.query(Tweet).all()

    liked_by_following = []  # Твиты с лайками от подписок
    other_tweets = []  # Остальные твиты

    for tweet in all_tweets:
        # Считаем лайки от подписок
        likes_from_following = sum(
            1 for like in tweet.likes if like.user_id in following_ids
        )
    ...
    # Твиты с лайками от подписок: сортировка по количеству лайков (больше = выше)
    liked_by_following.sort(key=lambda x: x[1], reverse=True)

    # Остальные твиты: сортировка по ID (новые = выше)
    other_tweets.sort(key=lambda t: t.id, reverse=True)

    # Объединяем: сначала залайканные подписками, потом остальные
    sorted_tweets = [t[0] for t in liked_by_following] + other_tweets
